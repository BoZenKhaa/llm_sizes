...
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  @media (max-width: 700px) {{
    ...
    /* In fullscreen on narrow screens the flex-column layout collapses
       the chart and pushes the static legend behind Plotly's stacking
       context. Restore the absolute overlay so the key floats above
       the chart. */
    body.chart-fullscreen #chart-wrap {{ display:block; }}
    body.chart-fullscreen .legend-panel {{
      position:absolute; order:0;
      top:44px; right:8px; left:8px;
      max-width:none;
      max-height:calc(100vh - 60px); overflow:auto;
      margin:0; box-shadow:0 6px 18px rgba(0,0,0,0.18);
  ...
  /* On touch devices, hide Plotly's hover tooltip via CSS rather than
     ...
     detection, which breaks plotly_click on taps. The modebar is also
     hidden since pinch/pan/double-tap/tap-to-fullscreen cover all of
     its functions and its buttons are tiny tap targets. */
  @media (hover: none) {{
    #chart-wrap .hoverlayer {{ display: none !important; }}
    #chart-wrap .modebar-container,
    #chart-wrap .modebar {{ display: none !important; }}
  }}
